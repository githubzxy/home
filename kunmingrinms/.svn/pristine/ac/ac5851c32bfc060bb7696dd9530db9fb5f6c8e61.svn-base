<%@page import="com.enovell.yunwei.attach.domain.UploadFile"%>
<%@page import="com.enovell.yunwei.attach.domain.AttachFile"%>
<%@page import="java.text.SimpleDateFormat"%>
<%@page import="com.enovell.yunwei.fileSystemNew.domain.FileSystemNew"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.enovell.yunwei.attach.dto.UploadFileDto,java.util.List"%>
<%             
	String contextPath = request.getContextPath();
	FileSystemNew m = (FileSystemNew)request.getAttribute("messageData");
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	String messageDate = sdf.format(m.getCreateDate());
%>
<!DOCTYPE html>
<html>
<head>
...
<!--                     </li> -->
        ...
        <div class="container detailDiv">
            <div class="module_title">
                <span>&nbsp;<a href="<%=contextPath%>/index.jsp">首页</a>/<a href="javascript:history.back(-1)">返回</a>/技术规章</span>
                <span></span>
            </div>
			<div class="detailDiv_basic_content">
				<h1>${messageData.fileName}</h1>
			</div>
			<div class="detailDiv_basic_content">
				<p class="info">
					<small>发布时间:<%=messageDate%></small>&nbsp;&nbsp;&nbsp;&nbsp; 
					<small>发布部门:${messageData.organization.name}</small>
				</p>
			</div>
			<div class="detailDiv_file">
			<%
			AttachFile a = m.getAttachFile();
			if(a != null){
				for(UploadFile f : a.getFiles()){
			%>
				<div class="file_show"><a href="<%=contextPath%>/homeAction/download.cn?path=<%=f.getPath()%>&fileName=<%=f.getName()%>" title="<%=f.getName()%>"><%=f.getName()%></a><div/>  
			<%
				}}
			%>
			</div>
