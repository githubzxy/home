...
<!--                     </li> -->
        ...
        <div class="big-table">
			<div class="module_title">
				<span class="title"><a href="<%=contextPath%>/index.jsp">返回</a>/<label id="nowDate"></label>值班情况</span>
				<span class="search"><input id="searchDate" /><button class="button button-info" onclick="searchForList()">搜索</button></span>
			</div>
...
function changeNowDate(d){
	var dateArr = d.split('-');
	if(!dateArr[1])
		dateArr = getToday().split('-');
	var result = dateArr[0] + '年' + dateArr[1] + '月' + dateArr[2] + '日';
	$('#nowDate').text(result);
}
...
BUI.use(['bui/calendar','bui/list','bui/data','bui/toolbar'],function(Calendar,List,Data,Toolbar){
	...
	// 渲染值班详情
	var listTpl = '<li><a>{theme}</a></li>';
	store = new Data.Store({
		url: contextPath+'homeAction/showOnduty.cn',
		proxy : {
			method : 'post',
			dataType : 'json'
		},
...
function searchForList(){
	...
	store.load({dutyDate: selectedDate},function(){
		changeNowDate(selectedDate);
	});
}
</script>
</html>