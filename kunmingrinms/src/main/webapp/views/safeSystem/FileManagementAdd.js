 ...
 * 文件新增模块
 */
define('views/safeSystem/FileManagementAdd',['bui/common',
	...
	var FileManagementAdd = BUI.Overlay.Dialog.extend({
		...
		_selectAutoFilling:function(){
			$("#fileTypeAdd").append("<option value='3110109' >安全规章</option>");
		...
		_initFormContainer: function(){
			...
			var childs = [{
				...
				redStarFlag: true,
				itemColspan: 2,
				item: '<input type="text" name="fileName" id="fileNameAdd" style="width:99.5%;" data-rules="{required:true}"/>'
			},{
				label: '规章制度类型',
				redStarFlag: true,
				itemColspan: 2,
			...
			},
			{
				label: '附件',
				itemColspan: 2,
				item: '<div name="attachFileId" id="uploadFileAddDiv" style="overflow-y:auto;height:100px;"></div>'
			},{
				label: '备注',
				itemColspan : 2,
				item : '<textarea name="remark" id="remarkAdd" style="width:99.5%;height:100px;" data-rules="{maxlength:200}" placeholder="最多输入200字"/>'
			}];
	        ...
	        success: {
	        	value: function(){
	        		...
	        		var postLoad = new PostLoad({
						url : contextPath + 'safeSystemAction/addInfor.cn',
						el : '#fileAddDialog',
						loadMsg : '保存中...'
					}); 
