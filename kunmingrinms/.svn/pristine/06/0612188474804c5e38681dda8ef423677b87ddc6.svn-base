...
BUI.use(['bui/overlay','bui/form'],function(Overlay,Form){
	...
	dialog = new Overlay.Dialog({
       ...
       loader:{
    	   ...
    	   url : contextPath + 'views/autumn/report_9_tongXinDuan_base_update.jsp',   
    	   dataType : 'html',
    	   autoLoad : false,          
           lazyLoad : false           
       },   
...
function getTable1(){
	$.ajax({
		type: "POST",
		url: "/kunmingrinms/LineImproveCollection?method=queryAllForTXD_Write",
		dataType: "json",
		data: {
			"year":$("#year").html(),
			"orgId":$("#orgId").val()
...
function autumn_year_table_execl(){
	...
	var url = "/kunmingrinms/LineImproveCollection?method=exportTXDForWrite";
	url += "&year=" + $("#year").html();
	url += "&orgId=" + $("#orgId").val();	
