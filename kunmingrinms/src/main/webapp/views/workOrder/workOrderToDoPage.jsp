...
// 					dialogClose(showDialog);
	...
	store = new Data.Store({
		pageSize : 10,//每页数量
		proxy : {
			url : contextPath + "workOrderTransactorAction/getByDTO.cn",//TODO异步数据源请求
			method : 'post'
		}
	});
	/*
...
//     		disposalBtn(record.id,record.status);
    	} else if(target.hasClass('replyBtn')) {
    		replyBtn(record.id);
    	} else if(target.hasClass('assign')){
    		assignFunc(record.id,rootId);
    	} else if(target.hasClass('btn-dispatch')){
    		turnToSendFunc(record.id,record.status);
    	} else if(target.hasClass('btn-show')){
...
// 	 * 处理按钮
// 	 */
// 	function disposalBtn(workOrderTransactorId,status) {
// 		var disposalDialog = disposalDialogCreate(workOrderTransactorId,status);
// 		disposalDialog.show();
...
// 	}
	 ...
	 * 转派
	 */
	function turnToSendFunc(workOrderTransactorId,status) {
...
//             var receiveUserId = $("#user").val();//接收人id
            var receiveUserId = e.data;//接收人id
            if(!receiveUserId){
            	tooltip("接收人不能为空");
                $(".bui-ext-mask").addClass("maskFour");
                return ;
