...
<script type="text/javascript">
	...
	function initPage(){
		$.ajax({
			type : "POST",
			url : "/kunmingrinms/QualityImproveSummary?method=getStatInfo",
			dataType : "json",
			//基础数据时通过部门，设备名称，年份唯一指定一条单相表中的记录
			data : {
				"deviceID" :  deviceID,
				"belongOrg" :  belongOrg,
				"year" :  year
			...
			success : function(response) {
				if (response.result != 1) {
					...
					//设置表单数据
					$("#addDiv #seq").html(formatNullData(sourceObj[0].SEQ_));
					$("#addDiv #device_name").html(sourceObj[0].DEVICE_NAME_);
					$("#addDiv #unit").html(sourceObj[0].UNIT_);
					$("#addDiv #total").html(sourceObj[0].TEST_IDENTIFY_NUM_);
					$("#addDiv #no_fit_num").html(sourceObj[0].NO_FIT_NUM_);
				}
			}
		});
	}
