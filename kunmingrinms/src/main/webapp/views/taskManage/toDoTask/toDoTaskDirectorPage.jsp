...
<body style="overflow: hidden;">
        ...
        <fieldset>
            ...
            <div class="row">
                ...
                <div class="control-group span8">
                    ...
                    <div class="controls" >
                        <select id="taskLevel" style="width: 180px;">
    ...
    <form action="/kunmingrinms/toDoTaskRootAction/expToDoByWorkShopDirector.cn" id="exportForm" method="post">
...
//      alert($(orgValInputId).val());
    ...
    BUI.use(["bui/grid","bui/data",'bui/calendar', 'bui/form','bui/overlay','bui/select','mod/GridBtn','common/multiSelectUser/MultiSelectUser','common/multiSelectUser/MultiSelectUserTool'],
            function(Grid,Data,Calendar, Form,Overlay,Select,GB,MultiSelectUser,MultiSelectUserTool){
        //日历
...
//      }));
        ...
        var columns = [{
                ...
                width:'10%',
                renderer:function(value){
                    return taskManage_taskLevel.typeName(value.taskLevel);
                }
            },{
                title:'任务类别',
                dataIndex:'root',
                ...
                width:'10%',
                renderer:function(value){
                    return taskManage_taskType.typeName(value.taskType);
                }
            },{
                title:'派发单位',
                dataIndex:'root',
                ...
                width:'15%',
                renderer: function(value) {
					return value.sendUser == null ? "" : value.sendUser.organization.name;
				}
            },{
                title:'派发时间',
                dataIndex:'root',
                ...
                width:'75px',
                renderer: function(value) {
					return value == null ? "" : value.limitTime;
				}
            },{
                title:'联系人',
                dataIndex:'root',
                ...
                width:'15%',
                renderer: function(value) {
					return  value.sendUser == null ? "" : value.sendUser.name;
				}
            },{
				title:'受令单位',
				dataIndex:'owner',
				...
				elCls : 'center',
				renderer: function(value) {
					return value.organization.name == null ? "" : value.organization.name;
				}
			},{
				title:'受令人',
				dataIndex:'owner',
                ...
                renderer:function(value,obj){
                	if((obj.owner.id == userId) && (value == taskManage_status.daiHuiFu )){
                	...
                	}else if((obj.owner.id == userId) && (value==taskManage_status.daiJieShou)){
                		return '<span class="grid-command btn-reply">回复</span>&nbsp;'+
                		'<span class="grid-command btn-assign">分派</span>&nbsp;'+
                		'<span class="grid-command btn-turn">转派</span>&nbsp;'+ 
                		'<span class="grid-command btn-back">退回</span>&nbsp;'+
                		'<span class="grid-command infoBtn">详情</span>';
                	}else if((obj.owner.id == userId) && (value==taskManage_status.yiFenPai)){
        ...
        store = new Data.Store({
            url : contextPath + 'toDoTaskRootAction/getToDoByWorkShopDirector.cn',
            pageSize : 10,
            proxy : {
                method : 'post',
                dataType : 'json'
        ...
        grid.on('cellclick',function  (ev) {
            ...
            if(target.hasClass('infoBtn')){
                handleDialog=handleDialogCreate(record.id, record.status,'任务详情');
                handleDialog.show();
             ...
             */
 			...
 			if(target.hasClass('btn-assign')){ //分派
 				...
 				$.post(contextPath + 'taskRootAction/getTaskRootTreeList.cn',{id:id},function(children){
 					 var isDaiJieShou=validateIsDaiJieShou(children);
// 	              console.log(isDaiJieShou);
 	                if(!isDaiJieShou){
 			...
 			if(target.hasClass('btn-turn')){ //转派
 				var multiSelectUserTool = new MultiSelectUserTool({
             		...
             		orgId:orgId,
             		orgName:orgName,
             		contextPath:contextPath+"toDoTaskRootAction/getSelectUsersByOrgId.cn",
             	});
             	...
             	multiSelectUserTool.on('sureBtn',function(e){
                     ...
                     $.post(contextPath + 'toDoTaskRootAction/turnToSendMultiUser.cn',
                             {
                                 id : id,
...
//                 }
                    ...
                    loader : {
                        url : contextPath + 'views/taskManage/toDoTask/toDoDetailAndChildren.jsp?id='+id+'&taskStatus='+taskStatus,
...
//                          console.log(checkMsg);
                            //提交后台
                            $.post(contextPath + 'taskRootAction/checkTask.cn',
                            {
                                id : id,
                    ...
                    loader : {
                        url : contextPath + 'views/taskManage/taskCheck/taskCheck.jsp?handleStatus='+handleStatus,
                        autoLoad : false,//不在弹出框组件初始化时自动加裁
                        lazyLoad : false,//不进行延迟加裁
                        dataType : 'html'//加载内容类型
                    }
...
//                          alert(sendBackRemark+",,,"+id);
                            $.post(contextPath + 'toDoTaskRootAction/sendBackToDoTask.cn',
                                    {
                                        id : id,
                    ...
                    loader : {
                        url : contextPath + 'views/taskManage/toDoTask/toDoBack.jsp',
                        autoLoad : false,//不在弹出框组件初始化时自动加裁
                        lazyLoad : false,//不进行延迟加裁
                        dataType : 'html'//加载内容类型
                    }
...
//                          console.log(receiveUser);
  					    ...
  					    if(!temp){
                            ...
                            $.post(contextPath + 'toDoTaskRootAction/assignToDoTask.cn',
                                    {
                                        id : id,
                    ...
                    loader : {
                        url : contextPath + 'views/taskManage/toDoTask/toDoAssign.jsp?id='+id,
                        autoLoad : false,//不在弹出框组件初始化时自动加裁
                        lazyLoad : false,//不进行延迟加裁
                        dataType : 'html'//加载内容类型
                    }
                ...
                replyDialog = new Overlay.Dialog({
                    ...
                    buttons : [ {
                        ...
                        handler :function(){
                            ...
                            //提交后台
                            $.post(contextPath + 'toDoTaskRootAction/replyToDoTask.cn',
                            {
                    ...
                    loader : {
                        url : contextPath + 'views/taskManage/toDoTask/toDoReply.jsp',
                        autoLoad : false,//不在弹出框组件初始化时自动加裁
                        lazyLoad : false,//不进行延迟加裁
                        dataType : 'html'//加载内容类型
                    }
        ...
        function exportXls(records){
            ...
            for(var i = 0 ; i < records.length ; i++){
                var row = records[i];
                json += '{'
                    +'"title":'+'"'+row.panrentTask.title+'",'
                    +'"taskLevel":'+'"'+taskManage_taskLevel.typeName(row.panrentTask.taskLevel)+'",'
                    +'"taskType":'+'"'+taskManage_taskType.typeName(row.panrentTask.taskType)+'",'
                    +'"org":'+'"'+row.panrentTask.sendUser.organization.name+'",'
                    +'"sendDate":'+'"'+Grid.Format.datetimeRenderer(row.panrentTask.sendDate)+'",'
                    +'"limitTime":'+'"'+Grid.Format.datetimeRenderer(row.panrentTask.limitTime)+'",'
                    +'"linkman":'+'"'+row.panrentTask.sendUser.name+'",'
                    +'"ownerOrg":'+'"'+row.owner.organization.name+'",'
