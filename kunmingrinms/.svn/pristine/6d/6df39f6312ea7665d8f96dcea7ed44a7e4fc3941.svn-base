define('views/commonjs/DetailTraining',[
...
//		利用公用组件展示文件
		_showByCommonController:function(result){
			...
			var viewUploader = new ViewUploader({
				...
				alreadyItems : result,
				previewUrl:_self.get('contextPath')+'views/accidentCommunicateNew/commonjsp/accidentCommunicatePageOffice.jsp'
			}); 
			viewUploader.render();
		},
			   ...
			   var postLoad =new PostLoad({
				...
				postLoad.load({attachFileId : downLoadFileId},function(result){
					...
					for(var i=0;i<result.length;i++){
						...
						// content += "<a href='/RINMS2MAIN/views/accidentCommunicateNew/commonjsp/accidentCommunicatePageOffice.jsp?filePath=" + result[i].path + "' target='_blank'>预览</a>";
                        content += "</span></div>";
		...
		bindUI:function(){
		   ...
		   $('.detailFormGridBtnForDetail').on('click',function(){
				...
				if(!selectionsLength){
					_self._showCommonFailure('请选择一条计录查看');
					return;
				}
				if(selectionsLength>1){
					_self._showCommonFailure('只能选择一条计录查看');
					return;
				}
		 ...
		 _packaging:function(url){
			 var _self=this;
				...
				window.location.href ='/RINMS2MAIN/DownloadFileServlet?fileName='+title+time+'.zip&filePath='+url;
		   },
