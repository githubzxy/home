...
<link href="<%=contextPath%>/resource/index/css/index-message.css" rel="stylesheet" />
</head>
<body>
<div class="main">
    <div class="banner">
...
<!--                     </li> -->
        ...
        <div class="big-table">
			<div class="module_title">
				<span class="title"><a href="<%=contextPath%>/index.jsp">首页</a>/电路工单</span>
				<span class="search"><input id="searchText" /><button class="button button-info" onclick="searchForList()">搜索</button></span>
			</div>
...
function calContentSize(){
	var rightWidth = $(window).width() - $('div.container.small').width() - 30;
	$('div.big-table').width(rightWidth);
}
//获取最新5条置顶的发布消息
...
var store;//store声明在全局是为了按标题查询使用
BUI.use(['bui/calendar','bui/list','bui/data','bui/toolbar'],function(Calendar,List,Data,Toolbar){
	// 渲染左下角的日历
	...
	calendar.render();
	// 隐藏日历按钮,因为日历用于查看，不用操作
	calendar.get('el').find('ul').hide();
	// 每行公告的页面模板
	var listTpl = '<li>[{createDate}]<a href="'+contextPath+'homeAction/showIndexCircuitWorkOrder.cn?id={docId}" target="_self">{workOrderName}</a></li>';
	store = new Data.Store({
		url: contextPath+'homeAction/getIndexCircuitWorkOrderList.cn',
		proxy : {
			method : 'post',
			dataType : 'json'
		},
	...
	var pagebar = new Toolbar.NumberPagingBar({
		render: '#pagebar',
