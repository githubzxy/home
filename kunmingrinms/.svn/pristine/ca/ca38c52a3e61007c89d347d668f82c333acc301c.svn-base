...
<body style="overflow: hidden;">
	...
	<form action="/kunmingrinms/circuitWOAction/expCircuitWO.cn" id="exportForm" method="post">
		...
		<input type="hidden" value="创建工单" name="fileName">
	</form>
...
// 			showTime : true,
		...
		store = new Data.Store({
			url : contextPath+'circuitWOAction/getAddListByDto.cn',
			pageSize : 10,
			proxy : {
				method : 'post',
				dataType : 'json'
...
//										||status==taskManage_status.shenHeTongGuo
									...
									commonFailure('可修改[草稿][待接收][审核不通过]的工单！');
								}
							}else{
...
// 						console.log(param);
						//提交数据至服务端
						$.post(contextPath + 'circuitWOAction/addAndSendCircuitWO.cn',
								{
...
// 						console.log(param);
						...
						//提交数据至服务端
						$.post(contextPath + 'circuitWOAction/addCircuitWO.cn',
								{
									param : JSON.stringify(param),
									receiveUser : JSON.stringify(receiveUser),
									carbonCopyUser : JSON.stringify(carbonCopyUser),
			...
			loader : {
				url : contextPath + 'views/circuitWOManage/circuitWOAdd.jsp',
       ...
       function editDialogCreate(id, status){
				...
				closeAction : 'destroy',//关闭时销毁加载到主页面的HTML对象
	   			...
	   			buttons : [ {
					...
					handler :function(){
						...
						//提交数据至服务端
						$.post(contextPath + 'circuitWOAction/updateAndSendCircuitWO.cn',
								{
									param : JSON.stringify(param),
									receiveUser : JSON.stringify(receiveUser),
									carbonCopyUser : JSON.stringify(carbonCopyUser),
			...
			loader : {
				url : contextPath +  'views/circuitWOManage/circuitWOEdit.jsp?id='+id,
				autoLoad : false,//不在弹出框组件初始化时自动加裁
				lazyLoad : false,//不进行延迟加裁
				dataType : 'html'//加载内容类型
			}
	...
	function updateEditData(param, receiveUser, carbonCopyUser, arr){
		$.post(contextPath + 'circuitWOAction/updateCircuitWO.cn',
				{
					param : JSON.stringify(param),
					receiveUser : JSON.stringify(receiveUser),
					carbonCopyUser : JSON.stringify(carbonCopyUser),
    ...
    function receiveUserDialogCreate(id, status){
    	receiveUserDialog = new Overlay.Dialog({
			...
			closeAction : 'destroy',//关闭时销毁加载到主页面的HTML对象
   			...
   			buttons : [ {
				...
				elCls : 'button btn-send',
				handler :function(){
					var receiveUser = editReceiveStore.getResult();//获取执行人信息
					if(receiveUser.length <= 0){
						tooltip("请添加执行人！");
						$(".bui-ext-mask").addClass("userMask");
						return ;
					...
					//提交数据至服务端
					$.post(contextPath + 'circuitWOAction/updateReceiveUser.cn',
							{
								id : id,
								receiveUser : JSON.stringify(receiveUser),
							},function(data,status){
			...
			loader : {
				url : contextPath +  'views/circuitWOManage/circuitWOReceiveUser.jsp?id='+id,
				autoLoad : false,//不在弹出框组件初始化时自动加裁
				lazyLoad : false,//不进行延迟加裁
				dataType : 'html'//加载内容类型
			}
       ...
       function checkDialogCreate(id){
    	   checkDialog = new Overlay.Dialog({
   			   ...
   			   listeners : {
   				...
   				buttons : [ {
					...
					handler :function(){
						...
						$.post(contextPath + 'circuitWOAction/sendToCheck.cn', 
							{
								id:id,
			...
			loader : {
				url : contextPath + 'views/circuitWOManage/circuitWOSendToCheck.jsp',
				autoLoad : false,//不在弹出框组件初始化时自动加裁
				lazyLoad : false,//不进行延迟加裁
				dataType : 'html'//加载内容类型
			}
    	   ...
    	   detailDialog = new Overlay.Dialog({
				...
				closeAction : 'destroy',//关闭时销毁加载到主页面的HTML对象
	   			...
	   			buttons : [{
					...
					handler : function() {
                        circuitWOReplyFileDownload(id, timeNow);
						$(".bui-ext-mask").addClass("userMask");
					}
					},{
						text : '关闭',
				...
				loader : {
					url : contextPath + 'views/circuitWOManage/circuitWODetail.jsp?id='+id+'&taskStatus='+taskStatus,
					autoLoad : false,//不在弹出框组件初始化时自动加裁
					lazyLoad : false,//不进行延迟加裁
					dataType : 'html'//加载内容类型
				}
       ...
       function sendData(id){
			...
			MLM.show();
    	  $.post(contextPath + 'circuitWOAction/sendCircuitWO.cn', 
			{id: id},
  		   	function(result,status){
				if(status){
					MLM.hide();
    	...
    	function deleteData(ids) {
    		...
    		BUI.Message.Confirm(msg,function(){
    			$.post(contextPath + 'circuitWOAction/deleteCircuitWOByIds.cn', { ids: ids},function(result,status){
    				if(status){
						commonSuccess('删除成功！');
					}else{
						commonFailure("删除失败！");
