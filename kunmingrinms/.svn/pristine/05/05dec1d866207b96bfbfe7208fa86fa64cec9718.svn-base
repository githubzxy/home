...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.enovell.yunwei.message.dto.IssueMessageDTO,com.enovell.yunwei.attach.dto.UploadFileDto,java.util.List"%>
<%             
	String contextPath = request.getContextPath(); 
	IssueMessageDTO message = (IssueMessageDTO)request.getAttribute("messageData");
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	String messageDate = sdf.format(message.getIssueTime());
%>
<!DOCTYPE html>
<html>
<head>
...
<!--                     </li> -->
        ...
        <div class="container detailDiv">
            <div class="module_title">
                <span>&nbsp;<a href="<%=contextPath%>/index.jsp">首页</a>/<a href="javascript:history.back(-1)">返回</a>/通知公示</span>
                <span></span>
            </div>
			<div class="detailDiv_basic_content">
				<h1>${messageData.theme}</h1>
			</div>
			<div class="detailDiv_basic_content">
				<p class="info">
					<small>发布时间:<%=messageDate%></small>&nbsp;&nbsp;&nbsp;&nbsp; 
					<small>发布部门:${messageData.issueOrgName}</small>
				</p>
			</div>
			<div class="detailDiv_file">
			<%
				IssueMessageDTO m = (IssueMessageDTO)request.getAttribute("messageData");
				List<UploadFileDto> files = m.getUploadFileArr();
				if(files != null){
					for(UploadFileDto f : files){
			%>
				<div class="file_show"><a href="<%=contextPath%>/homeAction/download.cn?path=<%=f.getPath()%>&fileName=<%=f.getName()%>" title="<%=f.getName()%>"><%=f.getName()%></a>
				</div>
			<%
					}
				}
