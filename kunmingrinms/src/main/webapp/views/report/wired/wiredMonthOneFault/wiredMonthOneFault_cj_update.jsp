...
<table border=0 cellpadding=0 cellspacing=0 width=3346 class=xl6723036
 ...
 <tr height=21 style='height:15.75pt'>
  ...
  src="/kunmingrinms/views/report/wired/wiredMonthOneFault/wiredMonthOneFault.files/image006.png"></span><span
  style=''>
  ...
  <td colspan=8 class=xl7123036 id="belongDepart">${sessionScope.user.organization.name }</td>
 ...
 <tr class=xl6723036 height=21 style='height:15.75pt'>
  ...
  <td colspan=2 class="xl7123036 fillEditCell" id="principal">${requestScope.result[0][61] }</td>
  <td class=xl6723036></td>
  <td class=xl6723036></td>
  <td class=xl6723036></td>
  ...
  <td colspan=2 class="xl8023036 fillEditCell" id="phone">${requestScope.result[0][65] }</td>
  <td class=xl6723036></td>
  <td class=xl6723036></td>
  <td class=xl6723036></td>
  <td class=xl6723036></td>
...
// 保存数据
function saveBtn() {
	var wiredMonthOneFaults = getSaveDate();
	
	$.post(contextPath + 'wiredMonthOneFaultAction/addOrUpdate.cn',
			{
				year : $("#year").val(),
				month : $("#month").val(),
				wiredMonthOneFaults :  JSON.stringify(wiredMonthOneFaults)
...
function getSaveDate() {
	...
	$("tr").each(function(i) {
		...
		if(rowArray && rowArray.length != 0) {
			...
			wiredMonthOneFaults[seq] = obj;
		}
	});
	
	return wiredMonthOneFaults;
}
	...
	$.post(contextPath + 'wiredMonthOneFaultAction/addOrUpdate.cn',
			{
				...
				if(status){
					$("#updateForm").attr("action", "/kunmingrinms/wiredMonthOneFaultAction/exportWorkShop.cn");
					$("#updateForm").submit();
				}else{
					alert("导出失败！");
				}
