 ...
 * 产品说明书新增模块
 */
define('views/technical/productInstructionAdd',
		['bui/common',
	...
	var ProductInstructionAdd = BUI.Overlay.Dialog.extend({
		// 初始化
		...
		bindUI: function(){
			...
			//添加产品说明书名称悬浮提示
    		$('#instructionNameAdd').on('mouseover',function(){
    			var str=$('#instructionNameAdd').val();
    			$('#instructionNameAdd').attr('title',str);
    		});
		},
		...
		_initFormContainer: function(){
			var form = new FormContainer({
				id: 'instructionFormAdd',
				colNum: 2,
				formChildrens: [{
					label: '产品说明书:',
					redStarFlag: true,
					itemColspan: 2,
					item: '<input type="text" name="name" id="instructionNameAdd" data-rules="{required:true}"/>'
				},{
					label: '资料类别:',
					redStarFlag: true,
					itemColspan: 2,
					...
					label: '附件:',
					itemColspan: 2,
					item: '<div name="uploadFileArr" id="uploadFileAddDiv" style="height:100px;overflow-y:auto"></div>'
				},{
					label: '备注:',
					itemColspan : 2,
					item : 
						'<textarea name="remark" id="remarkAdd" style="width:99.5%;height:100px;overflow-y :auto" maxlength="250" placeholder="最多输入250字"/>'
				}]
			});
			return form;
		},
		...
		_saveData:function(){
			var _self = this,contextPath = _self.get('contextPath');
    		var form = _self.getChild('instructionFormAdd');
    		//验证不通过
    		if(!form.isValid()){
    			return;
    		}
    		...
    		var postLoad = new PostLoad({
				url : contextPath + 'productInstruction/addProductInstruction.cn',
				el : _self.get('el'),
				loadMsg : '保存中...'
			}); 
