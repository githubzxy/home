...
<body style="overflow: hidden;">
	...
	<form action="/kunmingrinms/toDoTaskRootAction/expToDoTask.cn" id="exportForm" method="post">
		...
		<input type="hidden" value="退回任务管理" name="fileName">
	</form>
...
// 		}));
		...
		store = new Data.Store({
			url : contextPath+'taskRootAction/getShowDtoOfBackTask.cn',
			pageSize : 10,
			proxy : {
				method : 'post',
				dataType : 'json'
		...
		grid.on('cellclick',function  (ev) {
	       ...
	       function yiTuiHuiDialogCreate(id){
	    	   yiTuiHuiDialog = new Overlay.Dialog({
					...
					loader : {
						url : contextPath + 'views/taskManage/backTask/backTaskDetail.jsp?id='+id,
	     ...
	     //重派任务（二级弹出框）
	       function resendDialogCreate(id){
	    	   resendDialog = new Overlay.Dialog({
					...
					closeAction : 'destroy',//关闭时销毁加载到主页面的HTML对象
		   			...
		   			buttons : [{
						...
						handler : function() {
							...
							$.post(contextPath + 'taskRootAction/resendBackTask.cn',
									{
										id : id,
					...
					loader : {
						url : contextPath + 'views/taskManage/backTask/resendTask.jsp',
						autoLoad : false,//不在弹出框组件初始化时自动加裁
						lazyLoad : false,//不进行延迟加裁
						dataType : 'html'//加载内容类型
					}
    	...
    	function deleteData(ids) {
    		var msg = '您确定要删除该条任务吗?<br><span style="font-size:10px;">若该任务是最后一条子任务,则主任务的状态会自动变更为【草稿】或【已归档】</span>';
    		BUI.Message.Show({
				msg : msg,
				icon : 'question',
				closeable : false,
	     ...
	     function deleteByIds(id){
	    	 $.post(contextPath + 'taskRootAction/delBackTask.cn', 
    			 {
					id: id
