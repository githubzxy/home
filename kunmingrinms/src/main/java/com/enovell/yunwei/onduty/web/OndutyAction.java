...
@Controller
@RequestMapping(value = "/ondutyAction")
public class OndutyAction {
	...
	public ResultMsg addBoth(
		...
		try {
			...
			return ResultMsg.getSuccess("值班人员新增成功", dateStr);
		}catch(Exception ex) {
			 return ResultMsg.getFailure("值班人员新增失败", dateStr);
		}
	}
	...
	public ResultMsg update(
		...
		try {
			...
			return ResultMsg.getSuccess("值班人员修改成功", null);
		}catch(Exception ex) {
			 return ResultMsg.getFailure("值班人员修改失败", null);
		}
	}
	...
	public ResultMsg delete(
		...
		try {
			...
			return ResultMsg.getSuccess("删除成功！", ids);
		} catch (Exception e) {
			return ResultMsg.getFailure("删除失败！", ids);
		}
	}
			...
			while (it.hasNext()) {
				...
				if (!file.isEmpty()) {
					...
					try {
						...
						if(results.isEmpty()){
							response.sendRedirect("../views/onduty/onDutyAllPage.jsp?threePerId="+perId);
						}else{
