...
<table border=0 cellpadding=0 cellspacing=0 width=3346 class=xl6723036
 ...
 <tr height=37 style='height:27.75pt'>
  ...
  <td colspan=3 rowspan=3 class=xl8723036 style='border-right:1.0pt solid black;border-bottom:1.0pt solid black'>
  	<img width=211 height=112 src="/kunmingrinms/views/report/wired/wiredMonthOneFault/wiredMonthOneFault.files/image006.png">
  </td>
  ...
  <td rowspan=3 class=xl7623036 width=52 style='width:39pt'>一类障碍总数（件）</td>
  <td rowspan=3 class=xl7623036 width=52 style='width:39pt'>一类障碍延时（小时）</td>
...
BUI.use(['bui/data', 'bui/select'], function(Data, Select) {
	var store = new Data.Store({
		autoLoad : true,
		proxy : {
			url : contextPath + "wiredMonthOneFaultAction/getReportWorkshop.cn",
			method : 'post'
		},
		params : {
			year : $("#year").val(),
	...
	$("#searchBtn").click(function() {
		$("#countForm").attr("action", "/kunmingrinms/wiredMonthOneFaultAction/getSegmentByDto.cn");
		$("#countForm").submit();
	});
	
	$("#exportBtn").click(function() {

		$("#countForm").attr("action", "/kunmingrinms/wiredMonthOneFaultAction/exportSegment.cn");
		$("#countForm").submit();
	});
});
</script>
