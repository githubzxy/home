 ...
 * 文件管理主页面模块
 */
define('views/fileSystem/FileManagement',['bui/common',
	'bui/data','bui/grid','bui/calendar',
	'common/org/OrganizationPicker',
	'common/grid/SearchGridContainer',
	'common/data/PostLoad',
	'views/fileSystem/FileManagementAdd',
	'views/fileSystem/FileManagementUpdate',
	"views/fileSystem/SelectSuggest",
	'views/fileSystem/FileManagementDetails'],function(require){
	var BUI = require('bui/common'),
		Grid = require('bui/grid'),
		Data = require('bui/data'),
		Calendar = require('bui/calendar'),
		PostLoad = require('common/data/PostLoad'),
		SelectSuggest = require('views/fileSystem/SelectSuggest'),
		OrganizationPicker = require('common/org/OrganizationPicker'),
		SearchGridContainer = require('common/grid/SearchGridContainer'),
		FileManagementAdd = require('views/fileSystem/FileManagementAdd'),
		FileManagementUpdate = require('views/fileSystem/FileManagementUpdate'),
		FileManagementDetails = require('views/fileSystem/FileManagementDetails');
...
//						console.log(fileId);
					...
					if(selections.length != 0){
						...
						BUI.Message.Confirm('确认要删除么？',function(){
							var postLoad =new PostLoad({
								url:contextPath + 'fileSystemAction/delRecord.cn',
								loadMsg:''
							});
							data = {ids: fileIds};
			...
			_getFormObject: function(){
				/*return {
					...
					fileSuitableRange : $('#fileSuitableRange').val(),
...
//				console.log(data);
			...
			/*_exportXls:function(){
				...
				var postLoad = new PostLoad({
					url : contextPath + 'fileSystemAction/exportXls.cn',
					el : '#fileManage',
					loadMsg : '导出中...'
				}); 
					...
					items: [
						{
							...
							label : '作业指导书名称',
							item : '<input type="text" style="width: 175px;" name="fileName" id="fileName"/>'
						},{
							label : '专业类别',
							item : '<select name="fileType" id="fileType" style="width: 200px;"><option value="">请选择</option></select>'
//							item: '<div id="fileType" ></div>'
						},
						{
							label : '上传时间',
							item : '<input type="text" class="calendar" name="startDate" id="startDate" style="width: 175px;">'
						},{
							label : '至',
							item : '<input type="text" class="calendar" name="endDate" id="endDate" style="width: 175px;">'
						}
					]};
			},
			/**
...
//					render : '#fileType',//<div name="emsName" id="emsName" style="width: 191px;"></div>
//					name : 'fileType',
//					width : 191
//				});
...
//					suggest.set('tpl', '<input type="text" style="border-radius: 0px;"  placeholder="请选择正确的专业类别" autocomplete="off"/><span class="x-icon x-icon-normal" ><i class="icon icon-caret icon-caret-down"></i></span>');
////					suggest.set('width',130);
//					suggest.render();
//				});
//			},
			/**
			...
			_initFileType:function(){
				//初始化专业类别
	    		technicalInfo_type.listData("#fileType");
			},
			/**
			 * 表格列名
			 */
				...
				var columns = [
					{
						...
						renderer: function(value) {
    						return technicalInfo_type.typeName(parseInt(value));
    					}
					},
					{
						title:'所属部门',
			...
			_initStore: function(){
				...
				var store = new Data.Store({
					url: contextPath + 'fileSystemAction/getData.cn',
					autoLoad: true,
					pageSize: 10,
					proxy: {
						method: 'post',
