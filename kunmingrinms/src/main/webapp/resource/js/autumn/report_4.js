...
BUI.use(['bui/overlay','bui/form'],function(Overlay,Form){
	...
	baseDialog = new Overlay.Dialog({
       ...
       loader:{
    	   ...
    	   url : contextPath + 'views/autumn/report_4_base_update.jsp',   
    	   dataType : 'html',
    	   autoLoad : false,          
           lazyLoad : false           
       },   
...
function getTable1(){
	$.ajax({
		type: "POST",
		url: "/kunmingrinms/DeviceCollection?method=queryAll",
		dataType: "json",
		data: {
			"year":$("#year").html(),
			"orgId":$("#orgId").val()
...
function exportTable(){
	var url = "/kunmingrinms/DeviceCollection?method=export";
	url += "&year="+$("#year").html();
	url += "&orgId="+$("#orgId").val();
	window.location = url;
}
