			...
			<legend onclick="hiddenSearch()">查询</legend>
			  <div class="row">
			  	<div class="control-group span8">
					<label class="control-label">设备所属系统:</label>
					<div class="controls">
				...
				<div class="control-group span8">
					...
					<div class="controls">
						<select name="speciality" id="speciality" style="width: 181px;" disabled="disabled">
							<option value="">请选择</option>
						</select>
					</div>
				</div>
				<div class="control-group span8">
					<label class="control-label">板件名称:</label>
					<div class="controls">
						<input name="platesName" type="text" id="platesName" class="input-normal" style="width: 171px;" />
					</div>
				</div>
				<div class="control-group span8">
...
<!-- 			</div> -->
	...
	<form action="/kunmingrinms/dailySpare/sparePartAction/expSparePart.cn" id="exportForm" method="post">
...
// 	});
	...
	/***初始化设备属性下拉框****/
// 	daily_attribute.listData("#deviceProperties");
	
	function getSystemName(){
	 	$.post(contextPath + 'dailySpare/sparePartAction/getSystemName.cn',
				function(result){
						for(var i in result){
							$("#deviceProperties").append("<option value='" + result[i].value + "' >" + result[i].value+ "</option>");//初始化设备名称下拉选
					}
				},'json');
	} 
	...
	/**初始化下拉选开始*/	
// 	dailySpare_speciality.listData("#speciality");//初始化设备类别下拉选
...
// 	});
	...
	$("#deviceProperties").on("change",function(){
		if($('#deviceProperties').val()){
			$('#speciality').attr("disabled",false);
		}else{
			$('#speciality').attr("disabled",true);
		}
		$("#speciality").find("option[value!='']").remove(); //设备类别值变化，就清除设备名称select的所有option
		var systemName=$("#deviceProperties").val();
		if(systemName!=""){
	...
	function getTypeBySystemName(systemName){
	 	$.post(contextPath + 'dailySpare/sparePartAction/getTypeBySystemName.cn',
				...
				function(result){
						for(var i in result){
							$("#speciality").append("<option value='" + result[i].value + "' >" + result[i].value+ "</option>");//初始化设备名称下拉选
					}
				},'json');
	} 
...
// 				}
				...
				title:'板件名称',
				dataIndex:'platesName',
				elCls : 'center',
				width:150,
			},{
				title:'板件可用设备型号',
				dataIndex:'availableModle',
				elCls : 'center',
				width:150,
			},{
				title:'设备或单板编号',
...
// 			}
		...
		function addSparePart(param){
			console.log(param);
			$.ajax({//发送请求，保存数据
				url:contextPath +  'dailySpare/sparePartAction/addSparePart.cn',
				type:'post',
				data:{
...
//						alert(status);
		...
		function inSparePart(id,amountIn,comment){
			$.ajax({//发送请求，保存数据
				url:contextPath +  'dailySpare/sparePartAction/inSparePart.cn',
				type:'post',
				data:{
					id : id,
					amountIn : amountIn,
       ...
       function addDialogCreate(){
    	   addDialog = new Overlay.Dialog({
			...
			loader : {
				url : contextPath + 'views/dailySpare/sparePartsAdd.jsp',
...
//	    					dialogClose(uploadDialog);
					...
					loader : {
						url : contextPath + 'views/dailySpare/sparePartsImport.jsp',//弹出框加载的JSP页面
						autoLoad : false,//不在弹出框组件初始化时自动加裁
						lazyLoad : false,//不进行延迟加裁
						dataType : 'html'//加载内容类型
					}
     ...
     //备品入库
       function inDialogCreate(id){
			inDialog = new Overlay.Dialog({
				...
				loader : {
					url : contextPath +  'views/dailySpare/sparePartsIn.jsp?id='+id,
					autoLoad : false,//不在弹出框组件初始化时自动加裁
					lazyLoad : false,//不进行延迟加裁
					dataType : 'html'//加载内容类型
				}
     ...
     function updateSparePart(param){
    	 $.ajax({//发送请求，保存数据
				url:contextPath +  'dailySpare/sparePartAction/updateSparePart.cn',
				type:'post',
				data:{
					param : JSON.stringify(param)
					},
...
// 						console.log(validateParam);
				...
				loader : {
					url : contextPath +  'views/dailySpare/sparePartsEdit.jsp?id='+id,
					autoLoad : false,//不在弹出框组件初始化时自动加裁
					lazyLoad : false,//不进行延迟加裁
					dataType : 'html'//加载内容类型
				}
   				...
   				buttons : [ {
					...
					handler :function(){
						...
						if($("#outForm #amountNow_out").val()-$("#outForm #amountOut").val()<0){
							 BUI.Message.Alert("出库数量大于库存量！",'warning');
							return;
						}
						//获取参数
						...
						if(rearon.length>250){
							BUI.Message.Alert("使用原因过长，请控制在250字以内！",'warning');
							return;
						}
// 						alert(amountOut+","+rearon);
						//提交后台
						$.post(contextPath + 'dailySpare/sparePartAction/outSparePart.cn',
								{
									id : id,
				...
				loader : {
					url : contextPath +  'views/dailySpare/sparePartsOut.jsp?id='+id,
					autoLoad : false,//不在弹出框组件初始化时自动加裁
					lazyLoad : false,//不进行延迟加裁
					dataType : 'html'//加载内容类型
				}
   				...
   				buttons : [ {
					...
					handler :function(){
						...
						if($("#moveForm #amountMove_Now").val()-$("#moveForm #amountMove").val()<0){
							 BUI.Message.Alert("请输入正确的移库数量！",'warning');
							return;
						}
 						//获取参数
...
// 						alert(workShopId+","+workAreaId+","+amountMove);
						...
						if(workShopIdNow==workShopId&&workAreaIdNow==workAreaId){
							...
							$.post(contextPath + 'dailySpare/sparePartAction/moveSparePart.cn',
									{
										id : id,
				...
				loader : {
					url : contextPath +  'views/dailySpare/sparePartsMove.jsp?id='+id,
					autoLoad : false,//不在弹出框组件初始化时自动加裁
					lazyLoad : false,//不进行延迟加裁
					dataType : 'html'//加载内容类型
				}
...
// 						alert(amountReturn);
						if($("#returnForm #amountReturn_Now").val()-$("#returnForm #amountReturn").val()<0){
							 BUI.Message.Alert("请输入正确的返厂数量！",'warning');
							return;
						}
						//请求提交
						$.post(contextPath + 'dailySpare/sparePartAction/ReturnSparePart.cn',
								{
									id : id,
				...
				loader : {
					url : contextPath +  'views/dailySpare/sparePartsReturnFactory.jsp?id='+id,
					autoLoad : false,//不在弹出框组件初始化时自动加裁
					lazyLoad : false,//不进行延迟加裁
					dataType : 'html'//加载内容类型
				}
  				...
  				buttons : [ {
					...
					handler :function(){
						...
						if($("#scrapForm #amountScrap_Now").val()-$("#scrapForm #amountScrap").val()<0){
							 BUI.Message.Alert("请输入正确的报废数量！",'warning');
							return;
						}
						//获取参数
						...
						if(reasonScrap.length>250){
							BUI.Message.Alert("报废原因过长，请控制在250字以内！",'warning');
							return;
						}
						
// 						alert(amountScrap+","+reasonScrap);
						//请求提交
						$.post(contextPath + 'dailySpare/sparePartAction/scrapSparePart.cn',
								{
									id : id,
				...
				loader : {
					url : contextPath +  'views/dailySpare/sparePartsScrap.jsp?id='+id,
					autoLoad : false,//不在弹出框组件初始化时自动加裁
					lazyLoad : false,//不进行延迟加裁
					dataType : 'html'//加载内容类型
				}
	       ...
	       function useSituationDialogCreate(id){
	    	   useSituationDialog = new Overlay.Dialog({
					...
					closeAction : 'destroy',//关闭时销毁加载到主页面的HTML对象
		   			...
		   			buttons : [],
					loader : {
						url : contextPath +  'views/dailySpare/sparePartsUseSituation.jsp?id='+id,
						autoLoad : false,//不在弹出框组件初始化时自动加裁
						lazyLoad : false,//不进行延迟加裁
						dataType : 'html'//加载内容类型
					}
...
//     			alert(ids);
    			$.post(contextPath + 'dailySpare/sparePartAction/deleteSparePart.cn',
						{
							ids : ids
						},
						function(result,status){
