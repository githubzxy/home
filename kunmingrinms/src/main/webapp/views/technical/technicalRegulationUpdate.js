 ...
 * 技术规章修改模块
 */
define('views/technical/technicalRegulationUpdate',
		['bui/common',
	...
	function(require){
		...
		var TechnicalRegulationUpdate = BUI.Overlay.Dialog.extend({
			//初始化弹框控件
			...
			bindUI : function(){
				...
				//添加技术规章名称悬浮提示
	    		$('#regulationNameUpdate').on('mouseover',function(){
	    			var str=$('#regulationNameUpdate').val();
	    			$('#regulationNameUpdate').attr('title',str);
	    		});
	    		var buttons = [{
	    			text:'保存',
	    			elCls:'button',
			...
			_initUpdateData:function(){
				...
				postLoad.load({id : fileId},function(result){
					$('#regulationNameUpdate').val(result.name);
			...
			_initFormContainer:function(){
				var form = new FormContainer({
					id : 'regulationUpdateForm',
					colNum : 2,
					formChildrens : [{
						label: '规章名称：',
						redStarFlag: true,
						itemColspan: 2,
						item: '<input type="text" name="name" id="regulationNameUpdate" data-rules="{required:true}"/>'
					},{
						label: '资料类别：',
						redStarFlag: true,
						itemColspan: 2,
						...
						label: '附件：',
						itemColspan: 2,
						item: '<div name="uploadFileArr" id="uploadFileUpdateDiv"></div>'
					},{
						label: '备注：',
						itemColspan : 2,
						item :'<textarea name="remark" id="remarkUpdate" style="width:98.5%;height:95px;overflow-y :auto" maxlength="250" placeholder="最多输入250字"/>'
					}]
				});
				return form;
			},
			...
			_saveData:function(){
				var _self = this,contextPath = _self.get('contextPath'),fileId = _self.get('fileId');
        		var form = _self.getChild('regulationUpdateForm');
				// 验证不通过
		    	if(!form.isValid()) {
		    		 return;
		    	 }
			    ...
			    var param = {
			     ...
			     //执行提交到数据库的方法
			    var postLoad = new PostLoad({
					url : contextPath + 'technicalRegulation/updateTechnicalRegulation.cn',
					el : _self.get('el'),
					loadMsg : '保存中...'
				}); 
			...
			ATTRS : {
				id:{value:'technicalRegulationUpdate'},
				title : {value : '修改技术规章'},
				width : {value : 635},
				height : {value : 289},
				contextPath : {},
		...
		return TechnicalRegulationUpdate;
});