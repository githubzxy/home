	...
	var SearchGridContainer = Component.Controller.extend({//获取到按钮权限数据后再渲染表格
		...
		bindUI : function(){
			...
			searchForm.on('formSearch',function(e){
				if(_self.get('isJSONParam')=="true"){
					var param = e.param;
					store.load({param:JSON.stringify(param)});
				}else{
					var param = e.param;
					store.load(param);
				}
		...
		_calGridSize : function(){//根据窗口高度和上面查询区域高度计算下方列表高度和宽度
			...
			searchGrid.set('height', $(window).height() - TAB_HEIGHT - formHeight - extHeight*2);
		...
		ATTRS : {
			...
			store : {},
			searchGrid : {},
			height : {value : $(window).height() - 21},
			isJSONParam:{value:"false"}//quyy加 点击查询时表单中的数据是否以json对象格式传给后台 false_否  ture_是
		},
