...
function getTable1(){
	$.ajax({
		type: "POST",
		url: "/kunmingrinms/DeviceCollection?method=queryAllForTongXinDuanSearch",
		dataType: "json",
		data: {
			"year":$("#year").html(),
		...
		success: function(response){
			if(response.result!=1){
				...
				$.each(data,function(i,node) {	//遍历所有类型
						...
						html+="   <td>"+formatBRData(formatNullData(node.REMARK_))+"</td>";
						html+="</tr>";
				});
				$("#table1").append(html);
				$("#table1").append(base);
...
function exportTable(){
	var url = "/kunmingrinms/DeviceCollection?method=exportForTXD";
	url += "&year="+$("#year").html();
	url += "&orgId="+$("#orgId").val();
