package com.enovell.yunwei.fileSystem.web;
...
@Controller
@RequestMapping(value = "/fileSystemAction")
public class FileSystemAction {
	...
	public String getData(@RequestParam(value = "fileName",required=false) String fileName,
		...
		dtolist = fileSystemService.getAllFileSystems(dto, start, limit);
		
		count = fileSystemService.getAllFileSystemCount(dto);
	...
	public String getDataById(@RequestParam(value = "fileId",required=false) String fileId){
		...
		FileSystem result = fileSystemService.getFileSystemById(fileId);
	...
	public ResultMsg addInfor(@RequestParam(value = "fileName") String fileName,
		...
		try {
			//调用新增用户到数据库的方法
			fileSystemService.addFileSystem(fs,dtos);
	...
	public ResultMsg addInfor(FileSystemDTO fs,
		...
		try {
			//调用新增用户到数据库的方法
			fileSystemService.addFileSystem(fs, user);
			return ResultMsg.getSuccess("新增作业指导书成功！", null);
		} catch (Exception e) {
			e.printStackTrace();
			return ResultMsg.getFailure("新增作业指导书失败！", null);
		}
	...
	public ResultMsg updateRecord(@RequestParam(value = "id") String id,
		...
		FileSystem fs = fileSystemService.getFileSystemById(id);
		// 取出附件
		...
		try {
			// 修改技术图纸
			fileSystemService.updateFileSystem(fs,dtos);
			// 删除附件
	...
	public ResultMsg updateRecord(FileSystemDTO fs,
		...
		try {
			// 修改技术图纸
			fileSystemService.updateFileSystem(fs, user);
			return ResultMsg.getSuccess("修改作业指导书成功！", null);
		} catch (Exception e) {
			e.printStackTrace();
			return ResultMsg.getFailure("修改作业指导书失败！", null);
		}
	}
	...
	public ResultMsg delRecord(@RequestParam("ids") List<String> ids){
		...
		try {
			fileSystemService.deleteFileSystemsByIds(ids);
			return ResultMsg.getSuccess("删除作业指导书成功！", null);
		} catch (Exception e) {
			e.printStackTrace();
			return ResultMsg.getFailure("删除作业指导书失败！", null);
		}
	...
	public void exportXls(@RequestParam("exportXlsJson") String exportXlsJson,
		...
		List<FileSystemExportDTO> dataList= fileSystemService.getAllFile(fileSystemDTO);
		String[] headerTableColumns = new String[]{ 
				"作业指导书名称" + "_" +"80" + "_" + "my.getFileName()",
				"专业类型" + "_" +"25" + "_" + "my.getFileType()",
				"所属部门" + "_" +"15" + "_" + "my.getOrganization()",
		...
		CommonPoiExportExcel<FileSystemExportDTO> export = new CommonPoiExportExcel<FileSystemExportDTO>();
		export.exportXls("作业指导书管理", headerTableColumns, expandJexlContext, dataList, request, response);
	}
	
	/** 
	 * getDistinctProTypeName 获取去重后的专业类别集合
	 * @param type
	 * @return
	 * @author	liwt
	 */
	@RequestMapping(value = "/getDistinctProTypeList", method = RequestMethod.POST)
	@ResponseBody
	public String getDistinctProTypeName() {
		List<String>  result= fileSystemService.getProType();
		String json = JsonUtil.javaToJsonAsString(result);
		return json;
	}
}
