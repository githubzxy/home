...
BUI.use(["bui/grid","bui/data",'bui/tree', 'bui/extensions/treepicker', 'bui/form','bui/overlay','mod/GridBtn'],
	...
	},
	{
		...
		width:'25%',
		renderer:function(){
			 return '<span class="grid-command updateBtn">修改</span>&nbsp;'+
			 '<span class="grid-command deleteBtn">删除</span>';
		}
	}];
	store = new Data.Store({
		url : contextPath+'userInfoAction/getAllUser.cn',
		pageSize : 15,
		proxy : {
			method : 'post',
			dataType : 'json'
    ...
    function addDialogCreate(orgId){
 	   addDialog = new Overlay.Dialog({
			   ...
			   listeners : {
				...
				buttons : [ {
					...
					handler :function(){
						...
						// 执行提交到数据库的方法
						$.ajax({
							url : contextPath + 'userInfoAction/saveUser.cn',
							type : "post",
							data:{
			...
			loader : {
				url : contextPath + 'views/userInfoManager/userAdd.jsp?id=' + orgId,//弹出框加载的JSP页面
	...
	grid.on('cellclick',function  (ev) {
		...
		if(target.hasClass('deleteBtn')){
			BUI.Message.Confirm('确认要删除【' + record.userName + '】用户吗？',function(){
				$.post(
						contextPath + 'userInfoAction/deleteUser.cn',
    ...
    function editDialogCreate(id){
    	editDialog = new Overlay.Dialog({
			...
			loader : {
				url : contextPath + 'views/userInfoManager/userUpdate.jsp?id=' + id,//弹出框加载的JSP页面
				autoLoad : false,//不在弹出框组件初始化时自动加裁
				lazyLoad : false,//不进行延迟加裁
				dataType : 'html'//加载内容类型
			}
