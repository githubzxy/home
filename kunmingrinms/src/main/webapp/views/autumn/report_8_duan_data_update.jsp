...
<script type="text/javascript">
	...
	function initPage(){
		$.ajax({
			type : "POST",
			url : "/kunmingrinms/QualityImproveSummaryForDuan?method=getById",
			dataType : "json",
			data : {
				"suID" :  suID
			},
			success : function(response) {
				if (response.result != 1) {
					...
					//设置表单数据
					$("#updateDiv #seq").html(seq);
					$("#updateDiv #device_name").html(deviceName);
					$("#updateDiv #unit").html(unit);
					$("#updateDiv #total").html(total);
					$("#updateDiv #no_fit_num").html(noFitNum);
