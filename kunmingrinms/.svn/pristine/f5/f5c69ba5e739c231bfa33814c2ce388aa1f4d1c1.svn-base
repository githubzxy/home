			...
			if (response.result != 1) {
				...
				$.each(data, function(i, node) {
					...
					html += "   <td>" + formatNullData(node.DUTY_ORG_)
							+ "</td>";
					 html += " <td>" + formatNullData(node.DUTY_PEOPLE_) +
					 "</td>";
...
function autnmuTableExport() {
	var url = "/kunmingrinms/QualityImproveSummaryForDuan?method=detailAutumnExport";
	url += "&orgId=" + $("#orgId").val();
	url += "&year=" + $("#year").html();
