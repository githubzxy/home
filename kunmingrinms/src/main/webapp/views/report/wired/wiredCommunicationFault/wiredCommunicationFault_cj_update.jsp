...
<table border=0 cellpadding=0 cellspacing=0 width=1166 class=xl666057
 ...
 <tr class=xl666057 height=26 style='height:20.1pt'>
  ...
  <td class="xl726057 fillEditCell" id="principal">${requestScope.result[33] }</td>
  <td class=xl666057></td>
  <td class=xl666057></td>
  <td class=xl666057></td>
  <td class=xl666057></td>
  ...
  <td colspan=3 class="xl906057 fillEditCell" id="phone">${requestScope.result[34] }</td>
  <td class=xl666057></td>
  <td class=xl666057></td>
  <td class=xl666057></td>
  <td class=xl666057></td>
...
var resultLength='<%=resultLength%>';//返回数据的长度

$(document).ready(function(){
	initElementAttr();
	bindAllEvent();
	removeRowsByResult(resultLength);
});

//若返回的数据为空，删除可编辑行
function removeRowsByResult(resultLength){
	if(resultLength=='0'){
		$(".editRow").remove();
	}
}
...
function initElementAttr() {
	$(".editRow td:first").addClass("noEditCell");//给第一个单元格添加不可编辑的样式
...
// 	wiredCommFault[length+5]='${requestScope.result[29] }';//id
	...
	$.post(contextPath + 'wiredCommunicationFaultAction/add.cn',
			{
				...
				if(status){
					$("#updateForm").attr("action", "/kunmingrinms/wiredCommunicationFaultAction/exportWorkShop.cn");
					$("#updateForm").submit();
				}else{
					alert("保存失败!");
				}
