 ...
 * 文件修改模块
 */
define('views/safeSystem/FileManagementUpdate',
	...
	var FileManagementUpdate = BUI.Overlay.Dialog.extend({
		...
		_initUpdateData:function(){
			...
			var postLoad =new PostLoad({
				url:contextPath +  'safeSystemAction/getDataById.cn',
				el:'',
				loadMsg:''
			}); 
				...
				formChildrens : [
					{
						...
						redStarFlag : true,
						itemColspan : 2,
						item : '<input type="text" name="fileName" id="fileNameUpdate" style="width:99.5%;" data-rules="{required:true}"/>'+
					...
					},
					{
						label: '附件：',
						itemColspan: 2,
						item: '<div name="attachFileId" id="uploadFileUpdateDiv" style="overflow-y:auto;height:100px;"/>'
					},{
						label: '备注：',
						itemColspan : 2,
						item :'<textarea name="remark" id="remarkUpdate" style="width:99.5%;height:100px;" data-rules="{maxlength:200}" placeholder="最多输入200字"/>'
					}
				]
			});
			return form;
		...
		_selectAutoFilling:function(){
			$("#fileTypeUpdate").append("<option value='3110109' >安全规章</option>");
	        ...
	        success : {
	        	value : function(){
					...
					// 验证不通过
				     ...
				     //执行提交到数据库的方法
				     var postLoad = new PostLoad({
							url : contextPath + 'safeSystemAction/updateRecord.cn',
							el : '#fileUpdateDialog',
							loadMsg : '保存中...'
						}); 
						postLoad.load(data, function(result){
