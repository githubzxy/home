...
function getTable1(){
	$.ajax({
		type: "POST",
		url: "/kunmingrinms/DeviceCollectionDisaster?method=queryAll",
		dataType: "json",
		data: {
			"year":$("#year").html(),
			"orgId":$("#orgId").val()
...
function getBase1(){
	$.ajax({
		type: "POST",
		url: "/kunmingrinms/DeviceCollectionDisaster?method=getBase",
		dataType: "json",
		data: {
			"year":$("#year").html(),
			"orgId":$("#orgId").val()
		...
		buttons:[{
			...
			handler:function(){
				 ...
				 if(validate){
					 $.ajax({
							type: "POST",
							url: "/kunmingrinms/DeviceCollectionDisaster?method=addBase",
							dataType: "json",
							data : {
								"year" : $("#base_year").html(),
								"orgId" : $("#orgId").val(),
		...
		buttons:[{
			...
			handler:function(){
				 ...
				 if(validate){
					 $.ajax({
							type: "POST",
							url: "/kunmingrinms/DeviceCollectionDisaster?method=addCheck",
							dataType: "json",
							data: {
								"deviceId":deviceId,
								"year":$("#year").html(),
...
function exportTable(){
	var url = "/kunmingrinms/DeviceCollectionDisaster?method=export";
	url += "&year="+$("#year").html();
	url += "&orgId="+$("#orgId").val();
	window.location = url;
}
