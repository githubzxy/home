...
<body style="overflow: hidden;">
	...
	<form action="/kunmingrinms/taskRootAction/expTaskRoot.cn" id="exportForm" method="post">
		...
		<input type="hidden" value="全部任务查询" name="fileName">
	</form>
...
// 			showTime : true,
		...
		store = new Data.Store({
			url : contextPath+'taskRootAction/getAllTaskOfRootByDto.cn',
			pageSize : 10,
			proxy : {
				method : 'post',
				dataType : 'json'
...
// 							console.log(selections);
					...
					loader : {
						url : contextPath + 'views/taskManage/taskDetail.jsp?id='+id,
