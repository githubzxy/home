...
<body style="overflow: hidden;">
	...
	<form action="/kunmingrinms/dispatchRootAction/expToDoByWorkShopDirector.cn" id="exportForm" method="post">
...
// 		}));
	...
	var columns = [
		...
		},
		{
			...
			renderer : function(value,obj) {
				if((obj.user.id == userId) &&(value=='3'||value=='5')){//待接收 3
            	...
            	}else if((obj.user.id == userId) &&value=='7'){//已分派7
            		return'<span class="grid-command btn-assign">分派</span>&nbsp;'+
            		'<span class="grid-command btn-turn">转派</span>&nbsp;'+ 
            		'<span class="grid-command btn-show">详情</span>';
            	}else if((obj.user.id == userId) &&value=='4'){//处理中 4
	...
	//  数据库取出路局管理数据	
	store = new Store({
		url : contextPath + "dispatchRootAction/getToDoByWorkShopDirector.cn",
		pageSize : pageSize,
		proxy : {
			method : 'post',
			dataType : 'json',
			...
			if(target.hasClass('btn-turn')){ //转派
				var multiSelectUserTool = new MultiSelectUserTool({
               	...
               	multiSelectUserTool.on('sureBtn',function(e){
               		turnToSendMultiUser(id);// 转派
       				multiSelectUserTool.close();// 关闭转派弹出框
       			});
			}
	});
	...
	// 转派
	function turnToSendMultiUser(id){
	...
	function exportData(){
		...
		for(var i = 0 ; i < records.length ; i++){
			...
			json += '{'
				...
				+'"user":'+'"'+row.user.name+'",'
				+'"userOrg":'+'"'+row.user.organization.name+'",'
