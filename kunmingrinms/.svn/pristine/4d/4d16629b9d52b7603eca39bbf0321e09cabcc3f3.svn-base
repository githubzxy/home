...
BUI.use(["bui/grid","bui/data",'bui/calendar', 'bui/form','bui/overlay','bui/extensions/treegrid'],
	...
	function setParamDetail(data){
		...
		if(root){
			...
			$("#taskTypeDetail").val(taskManage_taskType.typeName(root.taskType));
			$("#taskLevelDetail").val(taskManage_taskLevel.typeName(root.taskLevel));
			if(root.sendUser){
			...
			$("#assignRemarkDetail").html(data.tempAssignRemark);
