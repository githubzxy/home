 ...
 * 
 * 项目名称：RINMS2MAIN
 * 类名称：circuitWOZipFileAction   
 * 类描述:  调令回复附件打包下载
 * @author lilt
 * @since 2018-10-29 10:34:52 AM
 */
@Controller
@RequestMapping(value = "/circuitWOZipFileAction")
public class CircuitWOZipFileAction extends ZipFileManager {
	...
	@RequestMapping(value = "/circuitOrderFilesToZip", method = RequestMethod.POST)
	@ResponseBody
	public String circuitOrderFilesToZip(HttpServletRequest request,
		...
		CircuitWO parent = new CircuitWO();
		parent.setId(parentId);
		List<CircuitWO> childList = treeService.getTaskRootTreeListForZip(parent);

		try {
			for (CircuitWO t : childList) {
	 ...
	 * @throws IOException
	 */
	private void querySubFile(String targetUrl, CircuitWO t) {

		Set<CircuitWO> children = t.getChildren();
	 ...
	 * @throws IOException
	 */
	private void dealWithFile(String targetUrl, CircuitWO t) {
