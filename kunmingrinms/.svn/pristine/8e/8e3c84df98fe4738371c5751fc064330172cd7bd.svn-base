...
<body style="overflow: hidden;">
	...
	<form action="/kunmingrinms/dailySpare/sparePartAction/expSpareAndNorm.cn" id="exportForm" method="post">
...
// 	});
	...
	//根据设备类别查询设备名称并初始化
	var speciality=daily_speciality.highSpeedEmergency;//设备类别为”高铁日常设备“
...
// 				pageSize : 10
	...
	BUI.use(["bui/grid","bui/data",'bui/calendar', 'bui/form','bui/overlay','bui/select', 'mod/GridBtn'],
		...
		store = new Data.Store({
			url : contextPath+'dailySpare/sparePartAction/getSpareAndNormByDto.cn',
			proxy : {
				method : 'post',
				dataType : 'json'
			},
...
// 				var row = records[i];
// 				json += '{'
// 					+'"speciality":'+'"'+dailySpare_speciality.typeName(row.speciality)+'",'
// 					+'"deviceName":'+'"'+row.deviceName+'",'
// 					+'"workShop":'+'"'+row.workShop+'",'
// 					+'"amount":'+'"'+row.amount+'",'
// 					+'"normValue":'+'"'+row.normValue+'",'
// 					+'"status":'+'"'+convertStatusToChinese(row.status)+'"'
// 					+'},';
// 			}
...
// 		}
		...
		function exportXls(){
			$("#exportXlsJson").val("");//清空值
// 			var obj = {
// 					speciality : dailySpare_speciality.typeName(speciality),
// 					deviceNameId : $("#deviceName").val(),
// 					workShopId : $("#workShop").val(),
